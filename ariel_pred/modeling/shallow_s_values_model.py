...
class ShallowModelTorch(nn.Module):
    def __init__(self, in_channels):
        super(ShallowModelTorch, self).__init__()
...
class ShallowModel:
    ...
    def _make_model(self):
        model = ShallowModelTorch(in_channels=self.in_channels).to(self.device)
        return model
    ...
    def train(
        ...
        predicted_spectra = np.zeros((len(data)))
    ...
    def _predict_from_single_model(
        ...
        predictions = np.zeros((len(data_loader.dataset)))  # type: ignore
        offset = 0
        running_loss = 0.0
        model.eval()
        with torch.no_grad():
    ...
    def predict(self, data: np.ndarray, batch_size: int = 32):
        ...
        predictions = np.zeros((len(model_paths), len(dataset), dataset.tensors[0].shape[1]))
