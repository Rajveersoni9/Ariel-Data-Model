...
def ariel_score(solution, submission, naive_mean, naive_sigma, sigma_true):
    ...
    GLL_true = np.sum(
        scipy.stats.norm.logpdf(y_true, loc=y_true, scale=sigma_true * np.ones_like(y_true))
    )
    GLL_mean = np.sum(
...
def score_single_row(
    ...
    if not isinstance(submission, np.ndarray):
        raise Exception("Submission must be a numpy array")
    if not isinstance(solution, np.ndarray):
        raise Exception("Solution must be a numpy array")
    if len(submission.shape) != 1:
        raise Exception("Submission must be a 1D array")
    if len(solution.shape) != 1:
        raise Exception("Solution must be a 1D array")

    n_wavelengths = len(solution)
    if len(submission) != n_wavelengths * 2:
        raise Exception("Wrong number of columns in the submission")
